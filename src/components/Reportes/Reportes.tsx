...
function Reportes() {
    let pacientes = JSON.parse(window.localStorage.getItem("listPacientes") as string);
    let cantidadF = 0;
    let cantidadM = 0;
    pacientes.map((paciente) => paciente.sexo === "Femenino" ? cantidadF++ : cantidadM++);
    let edades = Array(8).fill(0);
    const conteoEdades = [5, 6, 7, 8, 9, 10, 11, 12];

    pacientes.forEach((paciente) => {
        const edadIndex = conteoEdades.indexOf(paciente.edad);
        if (edadIndex !== -1) {
            edades[edadIndex]++;
        }
    });
    ...
    pacientes.forEach((paciente) => {
        paciente.fechaDiagnostico = paciente.fechaDiagnostico.split("-").reverse().join("-");
    ...
    const dataEdad = {
        labels: ['5 años', '6 años', '7 años', '8 años', '9 años', '10 años', '11 años', '12 años'],
        datasets: [
            {
                label: 'Edades',
                data: edades,
